...
var default_alipay_config = {
	...
	,create_direct_pay_by_user_return_url : '/alipay/create_direct_pay_by_user/return_url'  //即时到帐 同步跳转地址
	,create_direct_pay_by_user_notify_url: '/alipay/create_direct_pay_by_user/notify_url'   //即时到帐 异步通知回调地址
	,user_auth_quick_login_return_url:'/alipaylogin/return_url'                            //支付宝快捷登录 同步跳转地址
	,alipay_wap_create_direct_pay_by_user_return_url:'/alipay/alipay_wap_create_direct_pay_by_user/return_url' //支付宝手机网站支付同步跳转地址
	,alipay_wap_create_direct_pay_by_user_notify_url:'/alipay/alipay_wap_create_direct_pay_by_user/return_url' //支付宝手机网站支付异步回调地址
};
...
Alipay.prototype.route = function(app){
	...
	app.get(this.alipay_config.alipay_wap_create_direct_pay_by_user_return_url, function(req, res){self.alipay_wap_create_direct_pay_by_user_return(req, res)});
	app.post(this.alipay_config.alipay_wap_create_direct_pay_by_user_notify_url, function(req, res){self.alipay_wap_create_direct_pay_by_user_notify(req, res)});
}
...
Alipay.prototype.create_direct_pay_by_user = function(data, res){
	...
	var parameter = {
		...
		,seller_id:this.alipay_config.seller_id //卖家支付宝帐户 必填
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){                
				self.emit('create_direct_pay_by_user_trade_finished', _POST);
			}
			else if(trade_status == 'TRADE_SUCCESS'){                
				self.emit('create_direct_pay_by_user_trade_success', _POST);
			}
...
Alipay.prototype.create_direct_pay_by_user_return = function(req, res){		
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		...
		if(verify_result) { //验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){                
				self.emit('create_direct_pay_by_user_trade_finished',_GET,res);
			}
			else if(trade_status == 'TRADE_SUCCESS'){                
				self.emit('create_direct_pay_by_user_trade_success', _GET,res);
			}
...
Alipay.prototype.alipay_wap_create_direct_pay_by_user = function(data, res){
	assert.ok(data.out_trade_no && data.subject && data.total_fee);
	...
	var parameter = {
		...
		,partner:this.alipay_config.partner
		,payment_type:'1' //支付类型
		,notify_url: url.resolve(this.alipay_config.host, this.alipay_config.alipay_wap_create_direct_pay_by_user_notify_url)//服务器异步通知页面路径,必填，不能修改, 需http://格式的完整路径，不能加?id=123这类自定义参数
		,return_url: url.resolve(this.alipay_config.host , this.alipay_config.alipay_wap_create_direct_pay_by_user_return_url)//页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/
		,seller_id:this.alipay_config.seller_id //卖家支付宝帐户 必填
		,_input_charset:this.alipay_config['input_charset'].toLowerCase().trim()
	};
	for(var key in data){
...
Alipay.prototype.alipay_wap_create_direct_pay_by_user_notify = function(req, res){
	var self = this;
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){
				self.emit('alipay_wap_create_direct_pay_by_user_trade_finished', _POST);
			}
			else if(trade_status == 'TRADE_SUCCESS'){
				self.emit('alipay_wap_create_direct_pay_by_user_trade_success', _POST);
			}
			res.send("success");		//请不要修改或删除
		}
		else {
...
Alipay.prototype.alipay_wap_create_direct_pay_by_user_return = function(req, res){
	var self = this;
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		...
		if(verify_result) { //验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){
				self.emit('alipay_wap_create_direct_pay_by_user_trade_finished',_GET,res);
			}
			else if(trade_status == 'TRADE_SUCCESS'){
				self.emit('alipay_wap_create_direct_pay_by_user_trade_success', _GET,res);
			}
